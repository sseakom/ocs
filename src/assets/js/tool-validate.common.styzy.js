 ...
 * 使用说明：
 *  styzy.validate({
		selector: '#test', //选择器，选择当前节点下的所有有标识的input
		flagName: 'validate', //标识，插件只检测带有标识的input
		typeName: 'v-type', //验证类别，目前有 name、idcard、phone、mobile、email
		labelName: 'label', //输出信息的关键信息，姓名、手机号等
		passName: 'pass', //跳过标识，表示当前的input不需要验证
		//成功后的回调函数
		success: function() {
				toast('验证成功,全部通过','top');
			},
