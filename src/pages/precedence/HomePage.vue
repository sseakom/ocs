<template lang="jade">
	...
	div
		...
		vue-field(label="姓名", value="name", placeholder="请填写姓名", type="text", vType="name", root="precedence", disabled="disabled")
		vue-field(label="身份证号", value="cardID", placeholder="请填写身份证号", type="text", vType="idcard", root="precedence", disabled="disabled")
		.tips * 仅限满60周岁以上的居民办理。
...
export default {
	...
	mounted() {
		let self = this
		if (self.$store.state.precedence.dbr != '0') {
			//清空数据
			for (let key in self.$store.state.precedence) {
				self.$store.state.precedence[key] = ''
			}
			self.$store.state.precedence.dbr = '0' //设置代办
			self.$store.state.precedence.oid = '2c9a848157a8655d0157d1db74780305'
				//请求初始化信息
			self.$store.dispatch('getInitData').then(response => {
				self.$store.state.precedence.name = response.name
				self.$store.state.precedence.cardID = response.icard
				self.$store.state.precedence.tel = response.phoneNum
				self.$store.state.precedence.userid = response.userId
				self.$store.state.precedence.id = response.workSubmitId
					//渲染收件信息
				self.$store.state.precedence.applylinkname = response.name
				self.$store.state.precedence.applylinktel = response.phoneNum
					//获取注册信息
				self.$store.dispatch('getRegistry', self.$store.state.precedence.cardID).then(response => {
					self.$store.state.precedence.hjaddress = response.HJ
					self.$store.state.precedence.address = response.XJZDZ
					self.$store.state.precedence.applyaddr = response.XJZDZ
				})
			})
		}
	}
